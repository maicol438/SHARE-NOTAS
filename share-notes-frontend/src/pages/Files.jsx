  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-surface-100 mb-1">Archivos</h1>
          <p className="text-surface-500">{files?.length || 0} archivos</p>
        </div>
      ...
      {(!files || files.length === 0) ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {files.map((file) => {
            ...
            return (
              <div key={file._id || file.filename} className="bg-surface-900 border border-surface-800/60 rounded-xl p-4 hover:border-surface-700/60 transition-all relative group animate-fade-in">
                <button
                ...
                {file.type?.startsWith("image/") ? (
                  ...
                  <div className="w-full h-32 bg-surface-800 rounded-xl mb-3 flex items-center justify-center">
                    <Icon className="w-10 h-10 text-surface-400" />
                  </div>
                )}
                <p className="font-medium text-sm text-surface-100 truncate">{file.name}</p>
