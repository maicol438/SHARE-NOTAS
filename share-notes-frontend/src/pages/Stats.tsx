...
import { FileText, CheckSquare, Tag, Calendar, LucideIcon } from 'lucide-react';
...
interface StatsResponse {
  ...
  byCategory?: { category?: { _id: string; name: string }; count: number }[];
  byNotebook?: { notebook?: { _id: string; name: string }; count: number }[];
  byWeek?: { _id: string; count: number }[];
}
...
const BarChart: React.FC<BarChartProps> = ({ data }) => {
  ...
  const max: number = Math.max(...data.map((d) => d.count));

  return (
    <div className="space-y-3">
      {data.map((item: ChartDataItem, i: number) => (
...
const DonutChart: React.FC<DonutChartProps> = ({ total, completed }) => {
  ...
  const strokeDashoffset: number = circumference - (percent / 100) * circumference;
...
const LineChart: React.FC<LineChartProps> = ({ data }) => {
  ...
  const pathD: string = `M 0 ${height} ${data
    .map((d: ChartDataItem, i: number) => `L ${i * 50 + 25} ${height - (d.count / max) * height}`)
    .join(' ')} L ${(data.length - 1) * 50 + 25} ${height} Z`;

  const lineD: string = `M ${data
    .map((d: ChartDataItem, i: number) => `${i === 0 ? '' : 'L '}${i * 50 + 25} ${height - (d.count / max) * height}`)
    .join('')}`;
      ...
      <div className="flex justify-between mt-2 text-xs text-gray-500 dark:text-slate-500">
        {data.map((d: ChartDataItem, i: number) => (
          <span key={i} className="truncate">
            {typeof d.name === 'string' ? d.name.split('-W')[0] : `Sem ${i+1}`}
          </span>
        ))}
      </div>
    </div>
...
const Stats: React.FC = () => {
  ...
  const categoryData: ChartDataItem[] = (stats?.byCategory || [])
    .filter((s) => s?.category)
    .map((s) => ({
  ...
  const notebookData: ChartDataItem[] = (stats?.byNotebook || [])
    .filter((s) => s?.notebook)
    .map((s) => ({
  ...
  const tagData: ChartDataItem[] = stats.topTags?.map((t) => ({
    name: t._id,
    count: t.count,
  })) || [];
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white dark:bg-[#0d0b1f] border border-gray-200 dark:border-white/[0.06] rounded-2xl p-6">
          ...
          <LineChart data={(stats.byWeek || []).map(w => ({ name: w._id, count: w.count }))} />
        </div>
      </div>
