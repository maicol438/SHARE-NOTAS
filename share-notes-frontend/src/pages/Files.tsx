...
import { Upload, Image, FileText, X, Share2, LucideIcon } from 'lucide-react';
...
const Files: React.FC = () => {
  const { files, fetchFiles, deleteFile } = useNoteStore();
  ...
  const handleShareFile = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    if (!shareEmail.trim()) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      ...
      showToast('Error al subir archivos: ' + (axiosErr.response?.data?.message || axiosErr.message), 'error');
    } finally {
      setUploading(false);
      e.target.value = '';
    }
  ...
  const getIcon = (type: string | undefined): LucideIcon => {
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {(!files || files.length === 0) ? (
        <EmptyState
          ...
          onAction={() => { const el = document.querySelector<HTMLElement>('input[type="file"]'); if (el) el.click(); }}
        />
      ) : (
