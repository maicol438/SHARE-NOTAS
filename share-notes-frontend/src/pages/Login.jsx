...
import { Mail, Lock, BookOpen, Eye, EyeOff, ArrowLeft, Sparkles } from "lucide-react";
...
const Login = () => {
  ...
  useEffect(() => {
    // Check if there's a token in URL (from Google OAuth)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50 dark:bg-gray-950 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-purple-500/10 rounded-full blur-3xl animate-float delay-200" />
      </div>

      {/* Back button */}
      <Link to="/" className="absolute top-6 left-6 z-20 flex items-center gap-2 text-gray-500 hover:text-blue-600 transition-colors">
      ...
      <div className="w-full max-w-md relative z-10 animate-slide-up">
        ...
        <div className="card p-8 shadow-2xl animate-scale-in delay-100">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="relative input-with-icon">
              <div className={`icon transition-all duration-300 ${focused === "email" || form.email ? "top-3 -translate-y-0 text-xs" : ""} ${focused === "email" ? "text-blue-600" : "text-gray-400"}`}>
              ...
              <input
                ...
                placeholder={focused === "email" || form.email ? "" : "tu@email.com"}
                className={`input-field ${errors.email ? "border-red-500 ring-2 ring-red-500" : ""}`}
                autoComplete="email"
              />
            ...
            {/* Password */}
            <div className="relative input-with-icon">
              <div className={`icon transition-all duration-300 ${focused === "password" || form.password ? "top-3 -translate-y-0 text-xs" : ""} ${focused === "password" ? "text-blue-600" : "text-gray-400"}`}>
              ...
              <input
                ...
                className={`input-field pr-12 ${errors.password ? "border-red-500 ring-2 ring-red-500" : ""}`}
            ...
            <Button type="submit" isLoading={isLoading} className="w-full mt-2 py-3 text-lg rounded-xl shadow-lg shadow-blue-500/25 hover:shadow-xl hover:shadow-blue-500/40">
              Ingresar
            </Button>
            ...
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                ...
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18v2.84C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6 animate-fade-in delay-200">
          ¿No tienes cuenta?{" "}
          <Link to="/register" className="text-blue-600 hover:underline font-medium hover:text-blue-700">
            Regístrate gratis
          </Link>
        </p>
      </div>
