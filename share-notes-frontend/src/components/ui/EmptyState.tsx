import { Search, Star, Trash2, Plus, BookOpen, FolderOpen, Sparkles, Lightbulb, LucideIcon } from 'lucide-react';
import Button from './Button';

type EmptyStateType = 'notes' | 'search' | 'favorites' | 'trash' | 'category' | 'explore';
...
const defaultConfig: Record<EmptyStateType, EmptyStateConfig> = {
  notes: {
    icon: Sparkles,
...
export default function EmptyState({ type = 'notes' as EmptyStateType, title, subtitle, actionLabel, onAction, className = '' }: EmptyStateProps) {
  const config: EmptyStateConfig = defaultConfig[type] || defaultConfig.notes;
  const IconBg: LucideIcon = config.icon;
      ...
      {config.tips && config.tips.length > 0 && (
        <div className="w-full max-w-sm mb-8 space-y-2.5 z-10">
          ...
          <div className="grid gap-2 text-left">
            {config.tips.map((tip: string, i: number) => (
              <div 
                key={i} 
